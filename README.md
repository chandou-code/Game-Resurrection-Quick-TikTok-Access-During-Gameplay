...
- 确保在游戏中按键不会干扰到游戏操作。
- 使用时请注意游戏规则，避免影响游戏体验。
（激活了两次空格，第一次是空放，相当于占住坑位等进程完全开启，然后才能执行下一条的激活窗口并且发送空格 首先subprocess.Popen(file_path)打开进程默认的焦点在新开的进程上，但是如果后面跟了一个休眠那就会失去焦点，此时输入空格把焦点完全移走，再重新把激活窗口的焦点发送空格 这要不是我不小心多打一个函数谁能想得到 简直是生命般的巧合)
## 贡献
